...
const parseAdvice = (text) => {
  if (!text) return { well: "", risks: "", steps: "", outlook: "" };

  // Support both old and new Gemini response formats
  const wellMatch = text.match(
    /\*\*(?:What You Did Well|Strengths)[:\*]*\*?\*?([\s\S]*?)(?=\*\*(?:Key Risks|Concentration Risks|Rebalancing)|$)/i,
  );
  const risksMatch = text.match(
    /\*\*(?:Key Risks|Concentration Risks)[:\*]*\*?\*?([\s\S]*?)(?=\*\*(?:Rebalancing|Outlook)|$)/i,
  );
  const stepsMatch = text.match(
    /\*\*(?:Rebalancing Steps|Rebalancing Actions)[:\*]*\*?\*?([\s\S]*?)(?=\*\*Outlook|$)/i,
  );
  const outlookMatch = text.match(/\*\*Outlook[:\*]*\*?\*?([\s\S]*?)$/i);
    ...
    <div style={shellStyle}>
      ...
      <div style={containerStyle}>
        ...
        <div
          ...
          style={{ touchAction: "pan-y", paddingTop: "1rem" }}
        >
          {activeTab === "Overview" && (
            <>
              {/* ========== 1. SECTOR BREAKDOWN ========== */}
                    ...
                    <>
                      ...
                      <div style={{ display: "grid", gap: "0.75rem" }}>
                        ...
                        <div>
                          <p style={labelStyle}>
                            Sector Score:{" "}
                            {Number(diversification.sectorScore || 0).toFixed(
                              1,
                            )}
                            /10
                          </p>
                        ...
                        <div>
                          <p style={labelStyle}>
                            Size Score:{" "}
                            {Number(diversification.sizeScore || 0).toFixed(1)}
                            /10
                          </p>
                          <div
                            style={{
                      ...
                      <tbody>
                        {corrMatrixData.tickers.map((rowTicker, rowIndex) => (
                          <tr key={`row-${rowTicker}`}>
                            ...
                            {corrMatrixData.tickers.map(
                              (colTicker, colIndex) => {
                                const isDiag = rowIndex === colIndex;
                                const val = isDiag
                                  ? 1
                                  : toNumber(
                                      corrMatrixData.matrix?.[rowIndex]?.[
                                        colIndex
                                      ],
                                    );
